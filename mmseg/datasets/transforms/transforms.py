...
class PhotoMetricDistortion(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img, x_model = results['img'][:,:,:3],results['img'][:,:,3:6]
        # random brightness
        img = self.brightness(img)
        x_model = self.brightness(x_model)
        ...
        if mode == 1:
            ...
            x_model = self.contrast(x_model)

        # random saturation
        img = self.saturation(img)
        x_model = self.saturation(x_model)


        # random hue
        img = self.hue(img)
        x_model = self.hue(x_model)
        ...
        img = np.concatenate([img,x_model],-1)
