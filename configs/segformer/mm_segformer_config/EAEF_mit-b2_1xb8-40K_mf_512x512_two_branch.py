...
model = dict(
    ...
    data_preprocessor=dict(
        ...
        seg_pad_val=255,
        size=(480,640)),
    pretrained='D:/mmsegmentation/pretrain/bit_seg_mit_b2.pth',
    backbone=dict(
    ...
    decode_head =dict(
        ...
        loss_decode_similar_1 = [
            #dict(type='L1Loss', loss_weight=1.0),
            dict(type='PKDLoss', resize_stu=False, loss_weight=1.0)
        ],
        loss_decode_similar_2 = [
            #dict(type='L1Loss', loss_weight=1.0),
            dict(type='PKDLoss',resize_stu=False, loss_weight=1.0)
        ]
    ),
...
launcher = 'none'
work_dir = './work_dirs\\segformer_mf_3branch_mit-b2_bit'
